...
public class Hash4 {
    ...
    public static int[] solution(String[] genres, int[] plays) {
        ...
        for(int i = 0; i < genres.length; i++){
            map.put(genres[i], map.getOrDefault(genres[i], 0) + plays[i]); // map에는 장르별 전체 재생 횟수를 담는다.
        }

        // 1. 장르 선정
        ArrayList<String> genres_ordered = new ArrayList<>();
        while(map.size() != 0) { // map에 있는 내용들을 genres_ordered로 옮긴다.
            int max = -1;
            String max_key = "";
